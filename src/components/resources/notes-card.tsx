...
export function NotesCard() {
  ...
  const [selectedNoteRecipient, setSelectedNoteRecipient] = useState<{
    id: string;
    name: string;
  } | null>(null);
  const [showNoteRecipientSuggestions, setShowNoteRecipientSuggestions] =
    useState(false);
  const [noteSearchResults, setNoteSearchResults] = useState<any[]>([]);
  const [isSearchingNoteRecipients, setIsSearchingNoteRecipients] =
    useState(false);
  ...
  const searchNoteUsers = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `/api/coach/search?search=${encodeURIComponent(query)}&coachId=${user?._id}`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Crea y gestiona notas sobre tus sesiones de coaching, progreso de
          clientes y planes de acción.
        </CardDescription>
      </CardHeader>
        ...
        <Dialog open={showNoteDialog} onOpenChange={setShowNoteDialog}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Añade una nueva nota para un cliente y sesión específica.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid gap-2">
                ...
                {isLoadingSessions ? (
                  ...
                  <select
                    ...
                    onChange={e =>
                      handleNoteFormChange('sesion', e.target.value)
                    }
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                    disabled={
                      !selectedNoteRecipient || availableSessions.length === 0
                    }
                  >
                    <option value="">
            ...
            <DialogFooter className="flex justify-end gap-2">
              ...
              <Button onClick={handleCreateNote} disabled={isSendingNote}>
