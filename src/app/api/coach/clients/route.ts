...
export async function GET(request: NextRequest) {
  try {
    ...
    const clients = await Promise.all(
      coachProfile.clients.map(async (clientProfile: any) => {
        ...
        const nextSession = await Meet.findOne({
          ...
          .sort({ date: 1 })
          .select('date link objectiveId')
          .populate('objectiveId'); // Ordenar por fecha y hora ascendente
        ...
        const lastSession = await Meet.findOne({
          ...
          .sort({ date: -1 })
          .select('date link objectiveId')
          .populate('objectiveId'); // Ordenar por fecha y hora descendente
