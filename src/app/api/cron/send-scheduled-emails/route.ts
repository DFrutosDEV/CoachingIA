...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const cronIntervalMinutes = parseInt(process.env.CRON_INTERVAL_MINUTES || '1');
    ...
    console.log(`📧 Procesando emails programados entre ${startRange.toISOString()} y ${endRange.toISOString()}`);
    ...
    for (const emailDoc of pendingEmails) {
      ...
      try {
        console.log(`📤 Enviando email ${processedCount}/${pendingEmails.length} a ${emailDoc.to}`);
        ...
        console.error(`❌ Error enviando email a ${emailDoc.to}:`, errorMessage);
        
        // Marcar como fallido o reintentar
        if (emailDoc.canRetry()) {
          emailDoc.markAsFailed(errorMessage);
          await emailDoc.save();
          console.log(`🔄 Email marcado para reintento (${emailDoc.retryCount}/${emailDoc.maxRetries})`);
        } else {
          emailDoc.markAsFailed(errorMessage);
          await emailDoc.save();
          console.log(`💀 Email marcado como fallido permanentemente después de ${emailDoc.maxRetries} intentos`);
        }
      }
    ...
    console.log(`📊 Procesamiento completado: ${successCount} exitosos, ${errorCount} errores, ${processedCount} total`);
