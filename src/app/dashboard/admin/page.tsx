  ...
  const fetchBasicData = async () => {
    try {
      ...
      const response = await HttpClient.get(
        `/api/admin/getBasicData?adminId=${user?._id}`
      );
      ...
      if (smallCardsRef.current && !swapySmallRef.current) {
        try {
          ...
          swapySmallRef.current.onSwap((event: any) => {
            console.log(
              'Admin small cards swapped:',
              event.newSlotItemMap.asObject
            );
            localStorage.setItem(
              'adminSmallCardsLayout',
      ...
      if (largeCardsRef.current && !swapyLargeRef.current) {
        try {
          ...
          swapyLargeRef.current.onSwap((event: any) => {
            console.log(
              'Admin large cards swapped:',
              event.newSlotItemMap.asObject
            );
            localStorage.setItem(
              'adminLargeCardsLayout',
              JSON.stringify(event.newSlotItemMap.asObject)
            );
          });
        } catch (error) {
          ...
          <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-muted-foreground pt-2">
                    Gestiona usuarios, coaches y analiza el rendimiento de la
                    plataforma.
                  </p>
                </div>
                {/* <Button
            ...
            >
              <div data-swapy-slot="5" className="w-full">
                <NewUsersCard recentUsers={basicData?.recentUsers || []} />
              </div>
