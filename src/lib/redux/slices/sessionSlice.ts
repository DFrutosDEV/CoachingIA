...
const sessionSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSession: (state, action: PayloadAction<{ isLoggedIn: boolean; userType?: SessionState['userType'] }>) => {
      state.isLoggedIn = action.payload.isLoggedIn
      state.userType = action.payload.userType || null
      state.lastLoginDate = action.payload.isLoggedIn ? new Date().toISOString() : null
    },
