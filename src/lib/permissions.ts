...
export const dashboardRoutePermissions: RoutePermission[] = [
  ...
  },
  {
    ...
    allowedRoles:
      process.env.NODE_ENV === 'development' ? ['admin', 'coach'] : ['coach'],
    requiresAuth: true,
  ...
  },
  {
    ...
    allowedRoles:
      process.env.NODE_ENV === 'development' ? ['admin', 'client'] : ['client'],
    requiresAuth: true,
...
export function requiresAuthentication(pathname: string): boolean {
  ...
  console.log(
    `🔍 [${timestamp}] AUTH_CHECK: Verificando si ${pathname} requiere autenticación...`
  );
  ...
  if (isPublicRoute) {
    console.log(
      `✅ [${timestamp}] AUTH_CHECK: ${pathname} es una ruta pública`
    );
    return false;
  }
  ...
  console.log(
    `🔒 [${timestamp}] AUTH_CHECK: ${pathname} ${requiresAuth ? 'REQUIERE' : 'NO REQUIERE'} autenticación`
  );
...
export function hasRoutePermission(
  ...
  const timestamp = new Date().toISOString();
  console.log(
    `🔐 [${timestamp}] PERMISSION_CHECK: Verificando permisos de '${userRole}' para ${pathname}`
  );

  // Si es una ruta pública, permitir acceso
  if (!requiresAuthentication(pathname)) {
    console.log(
      `✅ [${timestamp}] PERMISSION_CHECK: Ruta pública, acceso permitido`
    );
    return true;
  }
  ...
  if (!matchingPermission) {
    console.log(
      `❌ [${timestamp}] PERMISSION_CHECK: No se encontró configuración para ${pathname}, denegando acceso`
    );
    return false;
  }
  ...
  console.log(
    `${hasAccess ? '✅' : '❌'} [${timestamp}] PERMISSION_CHECK: Usuario '${userRole}' ${hasAccess ? 'TIENE' : 'NO TIENE'} acceso a ${pathname}`
  );
  console.log(
    `📋 [${timestamp}] PERMISSION_CHECK: Roles permitidos: [${matchingPermission.allowedRoles.join(', ')}]`
  );
...
export function getRoutePermission(pathname: string): RoutePermission | null {
  const allPermissions = [...dashboardRoutePermissions, ...apiRoutePermissions];
  return (
    allPermissions.find(permission => pathname.startsWith(permission.path)) ||
    null
  );
}
