...
export function usePermissions(): UsePermissionsReturn {
  ...
  useEffect(() => {
    ...
    const getUserRole = () => {
      try {
        ...
        if (
          storedRole &&
          ['admin', 'coach', 'client', 'enterprise'].includes(storedRole)
        ) {
          console.log(
            `✅ [${timestamp}] HOOK: Rol encontrado en localStorage: '${storedRole}'`
          );
          return storedRole as UserRole;
        }

        console.log(
          `🔍 [${timestamp}] HOOK: Rol no encontrado en localStorage, buscando en token...`
        );
        ...
        if (token) {
          console.log(
            `🎫 [${timestamp}] HOOK: Token encontrado, decodificando...`
          );
          ...
          if (['admin', 'coach', 'client', 'enterprise'].includes(role)) {
            console.log(
              `✅ [${timestamp}] HOOK: Rol válido encontrado en token: '${role}', guardando en localStorage`
            );
          ...
          console.log(
            `❌ [${timestamp}] HOOK: Rol inválido en token: '${role}'`
          );
        } else {
          console.log(
            `❌ [${timestamp}] HOOK: No se encontró token en localStorage`
          );
        }
        ...
        console.error(
          `❌ [${timestamp}] HOOK: Error al obtener el rol del usuario:`,
          error
        );
    ...
    console.log(
      `👤 [${timestamp}] HOOK: Rol final obtenido: '${role || 'null'}'`
    );
    ...
    if (!role && typeof window !== 'undefined') {
      ...
      console.log(
        `🔍 [${timestamp}] HOOK: Verificando ruta actual: '${currentPath}'`
      );

      if (currentPath.startsWith('/dashboard')) {
        console.log(
          `🔄 [${timestamp}] HOOK: Usuario no autenticado en ruta protegida, redirigiendo a /login`
        );
        router.push('/login');
      } else {
        console.log(
          `✅ [${timestamp}] HOOK: Usuario no autenticado pero en ruta pública`
        );
      }
    } else if (role) {
      console.log(
        `✅ [${timestamp}] HOOK: Usuario autenticado con rol '${role}'`
      );
    }
  }, []);
  ...
  const hasPermission = (path: string): boolean => {
    ...
    if (!userRole) {
      console.log(
        `❌ [${timestamp}] PERMISSION: Sin usuario autenticado para verificar ${path}`
      );
      return false;
    }
    ...
    console.log(
      `🔐 [${timestamp}] PERMISSION: Usuario '${userRole}' ${hasAccess ? 'TIENE' : 'NO TIENE'} permisos para ${path}`
    );
  ...
  const getAllowedRoutes = (): string[] => {
    ...
    if (!userRole) {
      console.log(
        `❌ [${timestamp}] ROUTES: Sin usuario autenticado, devolviendo rutas vacías`
      );
      return [];
    }
    ...
    console.log(
      `📋 [${timestamp}] ROUTES: Rutas permitidas para '${userRole}':`,
      routes
    );
  ...
  const getRouteConfig = (path: string) => {
    ...
    console.log(
      `⚙️ [${timestamp}] CONFIG: Configuración para ${path}:`,
      config
    );
  ...
  const checkAndRedirect = (path: string): boolean => {
    const timestamp = new Date().toISOString();
    console.log(
      `🔍 [${timestamp}] CHECK: Verificando acceso y redirección para ${path}`
    );

    if (!userRole) {
      console.log(
        `🔄 [${timestamp}] CHECK: Sin usuario, redirigiendo a /login`
      );
      router.push('/login');
      return false;
    }
    ...
    if (!hasPermission(path)) {
      console.log(
        `🔄 [${timestamp}] CHECK: Sin permisos, redirigiendo a /dashboard/unauthorized`
      );
