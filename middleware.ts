...
export function middleware(request: NextRequest) {
  ...
  if (isProtectedRoute) {
    ...
    console.log('🔍 MIDDLEWARE DEBUG: Headers disponibles:', Array.from(request.headers.keys()));
    ...
    console.log(`👤 MIDDLEWARE: Usuario con rol '${userRole}' verificando permisos para ${pathname}`);
    
    // Verificar permisos específicos para rutas del dashboard
    if (pathname.startsWith('/dashboard/')) {
      if (!hasRoutePermission(pathname, userRole)) {
        console.log(`🚫 MIDDLEWARE: Usuario '${userRole}' sin permisos para ${pathname}`);
